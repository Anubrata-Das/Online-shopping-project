...
try{
	...
	ResultSet rs2=st.executeQuery("select * from users,cart where cart.email='"+email+"' and cart.status='bill'");
	while(rs2.next()){
...
<div class="left-div"><h3>Order Date: <%=rs2.getString(19) %> </h3></div>
<div class="right-div-right"><h3>Payment Method:<%=rs2.getString(21) %>  </h3></div>
<div class="right-div"><h3>Expected Delivery:<%=rs2.getString(20) %>  </h3></div> 

<div class="left-div"><h3>Transaction Id:  <%=rs2.getString(22) %></h3></div>
...
<h3>Product Details</h3>
  ...
  <%
  ResultSet rs1=st.executeQuery(" select * from cart,product where cart.product_id=product.id and cart.email='"+email+"' and cart.status='bill'");
  while(rs1.next()){
	  sno+=1;
